#ifndef KpMpii_20_hpp
#define KpMpii_20_hpp
...
// Same as Mpii_16 but with two additional keypoints for the feet
struct Mpii_20
{
...
class KpMpii_20 : public Pose
{
public:
   KpMpii_20() = default;
   KpMpii_20( std::string kpType,
      const std::map< KEYPOINT_TYPE, int > & kpLayout );
   KpMpii_20( const KpMpii_20 & rhs );
   virtual ~KpMpii_20(){}
   
   virtual Pose * Clone() const { return new KpMpii_20( *this ); }
...
inline const RigPose & KpMpii_20::RigPose() const
{
   return _rigPose;
}
inline RigPose & KpMpii_20::RigPose()
{
   return _rigPose;
}
inline void KpMpii_20::Keypoint( std::array< double, 3 > value, int index )
{
   *((std::array< double, 3 > *)&_keypoints + index) = value;
   _rigCreated = false;
}
inline void KpMpii_20::Keypoint( std::array< double, 3 > value, KEYPOINT_TYPE type )
{
   try
   {
...
inline const std::array< double, 3 > & KpMpii_20::Keypoint( KEYPOINT_TYPE type ) const
{
   return *((std::array< double, 3 > *)&_keypoints + _kpLayout.at( type ) );
}
inline std::array< double, 3 > & KpMpii_20::Keypoint( KEYPOINT_TYPE type )
{
   return const_cast< std::array< double, 3 > &>(static_cast<const Pose &>(*this).Keypoint( type ) );
}
inline bool KpMpii_20::HasKeypoint( KEYPOINT_TYPE type ) const
{
   return _kpLayout.count( type ) == 1;
}
inline const std::array< double, 3 > & KpMpii_20::CoordinateSystem() const
{
